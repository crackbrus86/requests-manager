...
if(current_user_can("edit_others_pages")):
    ...
    if(count($coachesId) > 0){
        foreach ($coachesId as $coachId) {
            $sql = $wpdb->prepare("SELECT DISTINCT phone FROM $tb_coaches WHERE id = %d", $coachId);
            $coach = $wpdb->get_row($sql);
            if($coach->phone && !in_array($coach, $coachPhoneNumber)) array_push($coachPhoneNumber, $coach);
        }
    }

    echo json_encode($coachPhoneNumber);
